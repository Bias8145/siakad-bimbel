...
import { Plus, Search, Edit2, Trash2, Mail, Phone, Upload, User, MapPin, MessageCircle, Eye, X, ChevronDown } from 'lucide-react';
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl md:text-3xl font-bold text-gray-900 tracking-tight dark:text-white">{t('students')}</h1>
          <p className="mt-1 text-sm text-gray-600 dark:text-gray-400">{t('manage_students')}</p>
        </div>
        <button
          onClick={() => { setSelectedStudent(null); setIsFormOpen(true); }}
          className="m3-btn m3-btn-primary shadow-xl shadow-blue-600/20 w-full md:w-auto"
        >
      ...
      {/* Search & Filter Bar */}
      <div className="glass-panel p-2 rounded-2xl flex items-center w-full bg-white dark:bg-[#1D1B20] border border-gray-200 dark:border-white/10">
        <div className="p-3 text-gray-400">
          <Search className="h-5 w-5" />
        </div>
        <input
          type="text"
          className="w-full bg-transparent border-none focus:ring-0 text-gray-700 dark:text-gray-200 placeholder-gray-400 outline-none"
      ...
      <div className="grid grid-cols-1 gap-4 md:hidden">
        {loading ? (
          ...
          filteredStudents.map((student) => (
            <div key={student.id} className="bg-white dark:bg-[#1D1B20] p-4 rounded-2xl shadow-sm border border-gray-100 dark:border-white/5 flex flex-col gap-3">
              <div className="flex items-start justify-between">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h3 className="font-bold text-gray-900 dark:text-white">{student.full_name}</h3>
                    <p className="text-xs text-gray-500">{student.grade_level} • {student.branch || 'Pusat'}</p>
                  </div>
                </div>
                <span className={`px-2 py-1 rounded-lg text-xs font-bold ${student.status === 'Active' ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
              ...
              <div className="grid grid-cols-2 gap-2 text-sm">
                <div className="bg-gray-50 dark:bg-white/5 p-2 rounded-lg">
                  ...
                  <p className="font-medium text-gray-800 dark:text-gray-200 truncate">{student.parent_name}</p>
                </div>
                <div className="bg-gray-50 dark:bg-white/5 p-2 rounded-lg">
                  <p className="text-xs text-gray-400">Email</p>
                  <p className="font-medium text-gray-800 dark:text-gray-200 truncate">{student.email}</p>
                </div>
              </div>
              ...
              <div className="flex gap-2 mt-1">
                 {student.parent_phone && (
                    <button 
                      ...
                      className="flex-1 py-2 bg-green-50 text-green-700 rounded-xl text-xs font-bold flex items-center justify-center gap-1"
                    >
                      <MessageCircle className="h-3.5 w-3.5" /> WA Wali
                    </button>
                 )}
                 <button onClick={() => handleViewDetail(student)} className="p-2 bg-gray-100 rounded-xl text-gray-600"><Eye className="h-4 w-4" /></button>
                 <button onClick={() => handleEdit(student)} className="p-2 bg-blue-50 rounded-xl text-blue-600"><Edit2 className="h-4 w-4" /></button>
                 <button onClick={() => handleDeleteClick(student.id)} className="p-2 bg-red-50 rounded-xl text-red-600"><Trash2 className="h-4 w-4" /></button>
              </div>
            </div>
          ))
        )}
      ...
      {/* Desktop View: Table */}
      <div className="hidden md:block glass-card overflow-hidden border border-gray-100 shadow-lg rounded-[32px]">
        <div className="overflow-x-auto">
          <table className="min-w-full divide-y divide-gray-100 dark:divide-white/5">
            ...
            <tbody className="divide-y divide-gray-100 dark:divide-white/5 bg-white dark:bg-[#1D1B20]">
              {loading ? (
                ...
                filteredStudents.map((student) => (
                  <tr key={student.id} className="hover:bg-blue-50/30 dark:hover:bg-white/5 transition-colors group">
                    ...
                    <td className="whitespace-nowrap px-4 py-5">
                      <span className="px-3 py-1.5 rounded-xl bg-gray-100 dark:bg-white/10 text-gray-700 dark:text-gray-300 text-sm font-semibold border border-gray-200 dark:border-white/10">
                        {student.grade_level}
                      </span>
                    </td>
                    <td className="whitespace-nowrap px-4 py-5">
                    ...
                    <td className="whitespace-nowrap px-4 py-5">
                      <div className="text-sm font-medium text-gray-900 dark:text-white">{student.parent_name}</div>
                    ...
                    <td className="relative whitespace-nowrap py-5 pl-4 pr-8 text-right text-sm font-medium">
                      <div className="flex justify-end gap-2">
                        <button 
                          ...
                          className="p-2 bg-gray-50 hover:bg-gray-100 rounded-lg text-gray-600 transition-colors border border-gray-200 dark:bg-white/5 dark:hover:bg-white/10 dark:border-white/10 dark:text-gray-300"
                          title="Lihat Detail"
                        >
                        ...
                        <button 
                          ...
                          className="p-2 bg-blue-50 hover:bg-blue-100 rounded-lg text-blue-600 transition-colors border border-blue-200 dark:bg-blue-900/20 dark:hover:bg-blue-900/40 dark:border-blue-800 dark:text-blue-300"
                          title={t('edit')}
                        >
                        ...
                        <button 
                          ...
                          className="p-2 bg-red-50 hover:bg-red-100 rounded-lg text-red-600 transition-colors border border-red-200 dark:bg-red-900/20 dark:hover:bg-red-900/40 dark:border-red-800 dark:text-red-300"
                          title={t('delete')}
                        >
...
// ... (StudentDetailModal and StudentModal components remain mostly same but ensure responsive padding)
function StudentDetailModal({ student, onClose }: { student: Student, onClose: () => void }) {
  // ... (Keep existing logic, just ensure classes are responsive)
  return (
    <div className="fixed inset-0 z-[100] overflow-y-auto">
      <div className="flex min-h-screen items-center justify-center px-4 pt-4 pb-20 text-center sm:p-0">
        ...
        <div className="relative inline-block transform overflow-hidden rounded-[32px] bg-white dark:bg-[#1D1B20] text-left align-bottom shadow-2xl transition-all w-full max-w-lg sm:align-middle border border-gray-100 dark:border-white/10">
           {/* Content same as before but adapted for dark mode if needed */}
          ...
          <div className="px-6 py-8">
             ...
             <div className="flex flex-col items-center mb-6">
                <div className="h-24 w-24 rounded-full bg-gray-100 overflow-hidden border-4 border-white dark:border-[#2B2930] shadow-lg mb-4">
                  {student.photo_url ? (
                    <img src={student.photo_url} alt={student.full_name} className="h-full w-full object-cover" />
                  ) : (
                ...
                <h2 className="text-2xl font-bold text-gray-900 dark:text-white text-center">{student.full_name}</h2>
                <span className="px-3 py-1 rounded-full bg-gray-100 dark:bg-white/10 text-gray-600 dark:text-gray-300 text-sm font-medium mt-2">
             ...
             <div className="space-y-4">
                <div className="bg-gray-50 dark:bg-white/5 p-4 rounded-2xl">
                   <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-3">Info Pribadi</h3>
                   <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                         <span className="text-xs text-gray-400 block">Email</span>
                         <span className="text-sm font-medium text-gray-900 dark:text-white break-all">{student.email || '-'}</span>
                      </div>
                      <div>
                         <span className="text-xs text-gray-400 block">Tgl Lahir</span>
                         <span className="text-sm font-medium text-gray-900 dark:text-white">{student.date_of_birth || '-'}</span>
                      </div>
                   </div>
                </div>
                <div className="bg-gray-50 dark:bg-white/5 p-4 rounded-2xl">
                   <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wider mb-3">Wali Murid</h3>
                   <div className="flex justify-between items-center">
                      <div>
                         <span className="text-xs text-gray-400 block">Nama</span>
                         <span className="text-sm font-medium text-gray-900 dark:text-white">{student.parent_name || '-'}</span>
                         <span className="text-xs text-gray-400 block mt-1">Telp</span>
                         <span className="text-sm font-medium text-gray-900 dark:text-white">{student.parent_phone || '-'}</span>
                      </div>
                      {student.parent_phone && (
                        <button 
                          onClick={() => openWhatsApp(student.parent_phone, `Halo.`)}
      ...
      <div className="flex min-h-screen items-center justify-center px-4 pt-4 pb-20 text-center sm:p-0">
        ...
        <div className="relative inline-block transform overflow-hidden rounded-[32px] bg-white dark:bg-[#1D1B20] text-left align-bottom shadow-2xl transition-all w-full max-w-lg sm:align-middle border border-gray-100 dark:border-white/10">
          <form onSubmit={handleSubmit(onSubmit)}>
            <div className="px-6 pt-8 pb-6 max-h-[80vh] overflow-y-auto">
              ...
              <div className="flex flex-col items-center mb-8">
                <div className="relative w-24 h-24 mb-4">
                  <div className="w-24 h-24 rounded-full overflow-hidden bg-gray-100 dark:bg-white/5 border-2 border-dashed border-gray-300 dark:border-gray-600 flex items-center justify-center group">
                    {photoPreview ? (
              ...
              <div className="grid grid-cols-1 gap-5">
                <div>
                  <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">{t('full_name')}</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                   <div>
                    <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">{t('dob')}</label>
                    <input type="date" {...register('date_of_birth', { required: true })} className="m3-input" />
                   </div>
                   <div>
                    <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">{t('grade_level')}</label>
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">{t('branch')}</label>
                ...
                <div className="pt-4 border-t border-gray-100 dark:border-white/10 mt-2">
                  ...
                  <div className="grid grid-cols-1 gap-4">
                     <div>
                      <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">{t('parent_name')}</label>
                      <input {...register('parent_name')} className="m3-input" placeholder={t('parent_name')} />
                     </div>
                     <div>
                      <label className="block text-sm font-bold text-gray-700 dark:text-gray-300 mb-1.5">{t('parent_phone')}</label>
            ...
            <div className="bg-gray-50 dark:bg-white/5 px-6 py-5 flex flex-row-reverse gap-3 border-t border-gray-100 dark:border-white/5">
              <button type="submit" disabled={saving || uploading} className="m3-btn m3-btn-primary px-8 py-2.5 text-sm w-full sm:w-auto">
                {saving ? t('saving') : t('save')}
              </button>
              <button type="button" onClick={onClose} className="m3-btn bg-white dark:bg-transparent text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-white/10 border border-gray-200 dark:border-white/10 px-6 py-2.5 text-sm w-full sm:w-auto">
                {t('cancel')}
              </button>
            </div>
          </form>
